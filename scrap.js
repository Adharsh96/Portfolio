...
let uploadedImageData = null; // Store resized base64 string
...
// Replace handleFileSelect with robust processImage
function processImage(file) {
    ...
    reader.onload = (e) => {
        ...
        img.onload = () => {
            ...
            // Resize to max 1000px width/height to ensure fast upload
            const maxDim = 1000;
            let width = img.width;
            ...
            uploadLabel.textContent = `selected: ${file.name} (compressed)`;
        };
...
// Updated drag & drop to use processImage
...
try {
    ...
    // Upload Image if selected (Use the resized base64 string)
    if (uploadedImageData) {
        console.log("Starting image upload (optimized)...");
        // Create a unique filename
        const filename = 'spills/' + Date.now() + '_img.jpg';
        const storageRef = ref(storage, filename);

        try {
            // Upload the base64 string (much faster and more reliable than raw file)
